(() => {
  ...
  const sx=x=>x-player.x+W/2, sy=y=>y-player.y+H/2;
  ...
  function drawPlayer(){
    ctx.save();ctx.translate(W/2,H/2);ctx.rotate(player.aim);if(player.invuln>0&&Math.floor(player.invuln*18)%2===0)ctx.globalAlpha=.3;ctx.shadowBlur=22;ctx.shadowColor=player.color;ctx.fillStyle='#081827';ctx.strokeStyle=player.color;ctx.lineWidth=2;
    ...
    if(state.roomGrace>0){ctx.strokeStyle=`rgba(57,246,255,${.25+state.roomGrace*.3})`;ctx.lineWidth=2;ctx.setLineDash([7,6]);ctx.beginPath();ctx.arc(W/2,H/2,48+Math.sin(state.totalTime*8)*3,0,TAU);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#39f6ff';ctx.textAlign='center';ctx.textBaseline='alphabetic';ctx.font='700 10px Consolas';ctx.fillText(`SAFE LINK ${state.roomGrace.toFixed(1)}s`,W/2,H/2-60);}
    else if(player.invuln>0){ctx.strokeStyle=`rgba(248,238,85,${.35+Math.min(1,player.invuln)*.45})`;ctx.lineWidth=3;ctx.shadowBlur=14;ctx.shadowColor='#f8ee55';ctx.beginPath();ctx.arc(W/2,H/2,34+Math.sin(state.totalTime*12)*3,0,TAU);ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle='#f8ee55';ctx.textAlign='center';ctx.textBaseline='alphabetic';ctx.font='900 9px Consolas';ctx.fillText(`INVULN ${player.invuln.toFixed(1)}s`,W/2,H/2-45);}
  }
  ...
  function render(){
    ctx.save();if(state.screenShake)ctx.translate((Math.random()-.5)*state.screenShake,(Math.random()-.5)*state.screenShake);drawWorld();
    if(state.current){
      ...
      for(const t of state.texts){ctx.globalAlpha=Math.max(0,t.life/t.maxLife);ctx.fillStyle=t.color;ctx.font=`${t.crit?'900':'800'} ${t.size}px Consolas`;ctx.shadowBlur=t.crit?16:5;ctx.shadowColor=t.color;ctx.fillText(t.text,sx(t.x),sy(t.y));if(t.crit){ctx.globalAlpha*=.4;ctx.strokeStyle='#fff';ctx.lineWidth=1;ctx.strokeText(t.text,sx(t.x),sy(t.y));}}ctx.shadowBlur=0;ctx.globalAlpha=1;ctx.textBaseline='alphabetic';drawMinimap();
    }
    ctx.restore();if(state.flash){ctx.fillStyle=`rgba(255,47,146,${state.flash*.7})`;ctx.fillRect(0,0,W,H);}if(state.transition){ctx.fillStyle=`rgba(57,246,255,${state.transition})`;ctx.fillRect(0,0,W,H);}
  }
  ...
  function bindTouchPad(element,isAim){let pointer=null;const stick=element.querySelector('i');const update=e=>{const r=element.getBoundingClientRect(),rawX=(e.clientX-(r.left+r.width/2))/(r.width*.5),rawY=(e.clientY-(r.top+r.height/2))/(r.height*.5),length=Math.hypot(rawX,rawY),scale=length>1?1/length:1,x=rawX*scale,y=rawY*scale;stick.style.transform=`translate(${x*r.width*.27}px,${y*r.height*.27}px)`;if(isAim){if(length>.12){touchInput.aimX=x;touchInput.aimY=y;}touchInput.aiming=true;mouse.down=true;}else{touchInput.moveX=x;touchInput.moveY=y;}};element.addEventListener('pointerdown',e=>{if(pointer!==null)return;pointer=e.pointerId;element.setPointerCapture(pointer);update(e);initAudio();e.preventDefault();});element.addEventListener('pointermove',e=>{if(e.pointerId===pointer){update(e);e.preventDefault();}});const end=e=>{if(e.pointerId!==pointer)return;pointer=null;stick.style.transform='translate(0,0)';if(isAim){touchInput.aiming=false;mouse.down=false;}else{touchInput.moveX=0;touchInput.moveY=0;}e.preventDefault();};element.addEventListener('pointerup',end);element.addEventListener('pointercancel',end);}
  bindTouchPad($('#move-pad'),false);bindTouchPad($('#aim-pad'),true);
  $('#start-button').onclick=openCharacterSelect;$('#restart-button').onclick=openCharacterSelect;document.querySelectorAll('.character-card').forEach(card=>card.onclick=()=>reset(card.dataset.character));$('#sound-button').onclick=e=>{soundOn=!soundOn;e.currentTarget.textContent=`SOUND: ${soundOn?'ON':'OFF'}`;if(soundOn){initAudio();startMusic();beep(440);}};$('#fullscreen-button').onclick=async()=>{try{if(!document.fullscreenElement)await document.documentElement.requestFullscreen();else await document.exitFullscreen();}catch{}};document.addEventListener('fullscreenchange',()=>{setTimeout(resize,80);$('#fullscreen-button').textContent=document.fullscreenElement?'EXIT FULL':'FULLSCREEN';});document.addEventListener('visibilitychange',()=>{if(document.hidden&&state.mode==='playing')togglePause();});if('serviceWorker'in navigator&&location.protocol.startsWith('http'))addEventListener('load',()=>navigator.serviceWorker.register('./service-worker.js').catch(()=>{}));resize();requestAnimationFrame(loop);
})();
