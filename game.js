(() => {
  ...
  function bindTouchPad(element,isAim){
    let pointer=null;const zone=element.parentElement,stick=element.querySelector('i');
    const update=e=>{const events=e.getCoalescedEvents?.(),sample=events?.length?events[events.length-1]:e,r=element.getBoundingClientRect(),max=r.width*.32,dx=sample.clientX-(r.left+r.width/2),dy=sample.clientY-(r.top+r.height/2),length=Math.hypot(dx,dy),strength=Math.min(1,length/max),x=length?dx/length*strength:0,y=length?dy/length*strength:0;stick.style.transform=`translate3d(${x*max}px,${y*max}px,0)`;if(isAim){if(strength>.1){touchInput.aimX=x;touchInput.aimY=y;touchInput.aiming=true;mouse.down=true;}else{mouse.down=false;}}else{touchInput.moveX=strength>.07?x:0;touchInput.moveY=strength>.07?y:0;}};
    const end=e=>{if(pointer===null||(e&&e.pointerId!==pointer))return;pointer=null;stick.style.transform='translate3d(0,0,0)';element.classList.remove('active');if(isAim){touchInput.aiming=false;mouse.down=false;}else{touchInput.moveX=0;touchInput.moveY=0;}e?.preventDefault();};
    zone.addEventListener('pointerdown',e=>{if(pointer!==null)return;const r=element.getBoundingClientRect(),distance=Math.hypot(e.clientX-(r.left+r.width/2),e.clientY-(r.top+r.height/2));if(distance>r.width*.64)return;pointer=e.pointerId;zone.setPointerCapture(pointer);element.classList.add('active');update(e);initAudio();e.preventDefault();});
