(() => {
  ...
  function musicTone(freq,duration,volume,type='sawtooth',slide=1){if(!soundOn||!audioCtx)return;const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type=type;o.frequency.setValueAtTime(freq,audioCtx.currentTime);o.frequency.exponentialRampToValueAtTime(Math.max(35,freq*slide),audioCtx.currentTime+duration);g.gain.setValueAtTime(volume*2,audioCtx.currentTime);g.gain.exponentialRampToValueAtTime(.0001,audioCtx.currentTime+duration);o.connect(g).connect(audioCtx.destination);o.start();o.stop(audioCtx.currentTime+duration);}
  function musicTick(){
  ...
  function bindTouchPad(element,isAim){
    let pointer=null,baseX=0,baseY=0;const zone=element.parentElement,stick=element.querySelector('i');
    const place=(clientX,clientY)=>{const r=zone.getBoundingClientRect(),radius=element.offsetWidth/2,edge=10;baseX=Math.max(radius+edge,Math.min(r.width-radius-edge,clientX-r.left));baseY=Math.max(radius+edge,Math.min(r.height-radius-24,clientY-r.top));element.style.left=`${baseX-radius}px`;element.style.top=`${baseY-radius}px`;element.style.bottom='auto';};
    const update=e=>{const events=e.getCoalescedEvents?.(),sample=events?.length?events[events.length-1]:e,r=zone.getBoundingClientRect(),radius=element.offsetWidth/2,max=radius*.64,dx=sample.clientX-(r.left+baseX),dy=sample.clientY-(r.top+baseY),length=Math.hypot(dx,dy),strength=Math.min(1,length/max),x=length?dx/length*strength:0,y=length?dy/length*strength:0;stick.style.transform=`translate3d(${x*max}px,${y*max}px,0)`;if(isAim){if(strength>.1){touchInput.aimX=x;touchInput.aimY=y;touchInput.aiming=true;mouse.down=true;}else{mouse.down=false;}}else{touchInput.moveX=strength>.07?x:0;touchInput.moveY=strength>.07?y:0;}};
    const end=e=>{if(pointer===null||(e&&e.pointerId!==pointer))return;pointer=null;stick.style.transform='translate3d(0,0,0)';element.classList.remove('active');element.style.removeProperty('left');element.style.removeProperty('top');element.style.removeProperty('bottom');if(isAim){touchInput.aiming=false;mouse.down=false;}else{touchInput.moveX=0;touchInput.moveY=0;}e?.preventDefault();};
    zone.addEventListener('pointerdown',e=>{if(pointer!==null)return;pointer=e.pointerId;zone.setPointerCapture(pointer);place(e.clientX,e.clientY);element.classList.add('active');update(e);initAudio();e.preventDefault();});
    zone.addEventListener('pointermove',e=>{if(e.pointerId===pointer){update(e);e.preventDefault();}});zone.addEventListener('pointerup',end);zone.addEventListener('pointercancel',end);zone.addEventListener('lostpointercapture',end);
  }
